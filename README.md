...
- **Pipeline (BERTopic):** GTE-large sentence embeddings → UMAP dimensionality reduction → HDBSCAN density clustering → class-based TF-IDF topic representation, followed by automated topic labeling using GPT-4.1-mini through the OpenAI Responses API. The resulting labels have not yet undergone comprehensive manual review.
...
| `08_topic_labeling` | Automated topic labeling using GPT-4.1-mini through the OpenAI Responses API. The resulting labels have not yet undergone comprehensive manual review. |
